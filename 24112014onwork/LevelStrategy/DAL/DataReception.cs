...
public class DataReception
{
    ...
    private void AddToTimer(List<DateTime> from, List<DateTime> to)
    {
            ...
            for (int i = 0; i < substrings.Length - 1; i = i + 4)
            {
                ...
                if (temp == null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            MainForm.grid.Rows.Add(listBars.Last().Name);
                            MainForm.grid.Rows[listBars.OfType<Bars>().Last().NumberGrid].Cells[0].ReadOnly = true;
                            MainForm.grid.Rows[listBars.OfType<Bars>().Last().NumberGrid].MinimumHeight = 35;
                        }));
