...
namespace LevelStrategy.BL
{
    public class Strategy
    {
        public static void FindSignal(Bars bars, EventHandler<SignalData> eventHandler)
        {
            ...
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Повторяющийся уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.High[bsy], bars.High[bsy] * 0.9996, ((bars.High[bsy] * 0.9996) * 0.996)));
            ...
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Зеркальный уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.Low[bsy], bars.Low[bsy] * 0.9996, ((bars.Low[bsy] * 0.9996) * 0.996)));
                eventHandler(new object(), bars.listSignal.Last());
            }
            if (AirLevel(bar, bars, out bsy) == "Short")// && DateStart(bar))//&& DefenitionAreaNearLevel(bar) == "Nothing" && ExceptionBars(bar) && shortTrade)
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Воздушный уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.Low[bsy], bars.Low[bsy] * 0.9996, ((bars.Low[bsy] * 0.9996) * 0.996)));
            ...
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Повторяющийся уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.Low[bsy], bars.Low[bsy] * 1.0004, ((bars.Low[bsy] * 1.0004) * 1.004)));
                eventHandler(new object(), bars.listSignal.Last());
            }
            ...
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Зеркальный уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.High[bsy], bars.High[bsy] * 1.0004, ((bars.High[bsy] * 1.0004) * 1.004)));
                eventHandler(new object(), bars.listSignal.Last());
            }
            if (AirLevel(bar, bars, out bsy) == "Long")//&& DateStart(bar))// && DefenitionAreaNearLevel(bar) == "Nothing" && ExceptionBars(bar) && longTrade)
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Воздушный уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.High[bsy], bars.High[bsy] * 1.0004, ((bars.High[bsy] * 1.0004) * 1.004)));
                ChangeColorConsole(true);
                eventHandler(new object(), bars.listSignal.Last());
                ChangeColorConsole(false);
            }
